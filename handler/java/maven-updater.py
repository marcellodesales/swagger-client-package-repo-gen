...
print("")
print("🐍 Custommizing '{}' with custom info".format(pomFile))
print("")
try:
  file = open(pomFile, 'r')
  print("Updating '{}' with parameters provided...".format(pomFile))
  ...
  if "scm" in element.tag:
    for scmElement in element:
      if "connection" in scmElement.tag or "developerConnection" in scmElement.tag:
        ...
        # Update the connection if the library does not include in the name already
        if os.environ["GENERATE_CLIENT_WITH_LIBRARY"] not in scmElement.text:
...
if "gitlab" in os.environ["GENERATE_CLIENT_GIT_HOST"]:
  ...
  shutil.copy2("/generator/cicd/gitlab/maven/ci_settings.xml", os.environ["CLIENT_API_LOCATION"])

  print("- Adding personal_settings.xml")
  shutil.copy2("/generator/cicd/gitlab/maven/personal_settings.xml", os.environ["CLIENT_API_LOCATION"])

  print("- Adding .gitlab-ci.yml")
  shutil.copy2("/generator/cicd/gitlab/maven/.gitlab-ci.yml", os.environ["CLIENT_API_LOCATION"])

  print("- Generating the maven repos in Maven for Gitlab")
  templateFile = "/generator/cicd/gitlab/maven/repos.xml"
  cicdFile = "{}/cicd-maven.xml".format(os.environ["CLIENT_API_LOCATION"])
  ...
  if os.getenv("CREATE_NEW_GIT_REPO"):
